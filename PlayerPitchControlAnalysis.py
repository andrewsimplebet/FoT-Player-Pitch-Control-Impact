...
class PlayerPitchControlAnalysisPlayer(object):
    def __init__(
        ...
        """
        This class is used to consolidate many of the functions that would be used to analyze the impact of a player's
            movement on pitch control
        Leveraging @EightyFivePoint's pitch control model as presented in the Friends of Tracking Series, we build out a
        ...
        Using an event from the match's event dataframe, and a specific team/player ID, we can:
            ...
            2. Calculate the difference in total space occupied by the player's team and plot the difference in pitch
                control surfaces with his/her current movement vs theoretical movement
            3. Calculate the difference in total space occupied by the player's team and plot the difference in pitch
                control surfaces if the player were not on the pitch at all
            4.  Calculate the difference in total space occupied by the player's team and plot the difference in pitch
                control surfaces if the player were in a different location on the pitch and containing a new velocity
                vector
    ...
    def calculate_pitch_control_new_location(
        ...
        if (replace_x_velocity == 0) & (replace_y_velocity == 0):
            warnings.warn("You have not specified a new velocity vector for the player. All analysis will assume "
    ...
    def calculate_pitch_control_difference(
        ...
        one player's attributes (velocity, position, or presence) is edited. In this function, you should use the
        arguments that are relevant to the parent pitch control calculate function.

        Input parameters:
        :param float replace_x_velocity: The x vector of the velocity we would like to replace our given player with.
            Positive values will move the player toward's the home team's goal, while negative values will move the
            player towards the Away Team's goal. Measured in  meters per second. Not used if
        ...
        :param float replace_y_velocity: The y vector of the velocity we would like to replace our given player with.
            ...
            away team. Measured in meters per second. Not used if ``replace_function = 'presence'`` Defaults to 0.
        :param float relative_x_change: Determines how much to move the player in the x direction,
            relative to his/her current location on the pitch. Measured in meters. Not used if `
            `replace_function = 'presence'`` or ``replace_function == 'movement'``.
        :param string replace_function: The type of function we are using to analyze the player.
        Must be either "movement", "presence" or "location".
        "movement" uses the function ``calculate_pitch_control_replaced_velocity`` to calculate a new pitch control surface
        :return:
            pitch_control_difference: Difference in pitch control surfacesPitch control surface
                (dimen (n_grid_cells_x,n_grid_cells_y) ) between when the relevant player uses his/her actual movement
                and his/her theorized movement
            xgrid: Positions of the pixels in the x-direction (field length)
            ygrid: Positions of the pixels in the y-direction (field width)
        """
        ...
        print(pitch_control_difference.max(), pitch_control_difference.min())
    ...
    def calculate_space_created(
        ...
        :param float replace_x_velocity: The x vector of the velocity we would like to replace our given player with. Positive values will move the player toward's the home team's goal, while negative values will move the player towards the Away Team's goal
        :param float replace_y_velocity: The y vector of the velocity we would like to replace our given player with. Positive values will move the player to the left side of the pitch, from the perspective of the away team, while negative values will move the player towards the right side of the pitch from the perspective of the away team
